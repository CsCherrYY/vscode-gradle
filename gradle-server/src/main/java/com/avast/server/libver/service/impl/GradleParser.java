...
public class GradleParser {
  ...
  private static final String PROJECT_TITLE =
      "------------------------------------------------------------\\n(.*?)\\n------------------------------------------------------------";
  private static final Pattern PROJECT_PATTERN = Pattern.compile(PROJECT_TITLE);
  ...
  private void fillSourceSets(Subproject subproject, String projectDepsData) throws ParseException {
    ...
    while (matcher.find(sourceIndex)) {
      ...
      String sourceSet =
          projectDepsData.substring(titleIndex + 2, projectDepsData.indexOf('\n', titleIndex + 3));
      ...
      try (SourceSetParser sourceSetParser =
          new SourceSetParser(projectDepsData.substring(treeLineStart))) {
  ...
  private class SourceSetParser implements AutoCloseable {
    ...
    String walkTree(Node currentNode, String currentLine) {
      ...
      while (currentLine != null) {
        final int foundPlus =
            currentLine.indexOf("---"); // well, not 100% optimized when going back in tree
        int lineLevel = 0;
        if (foundPlus >= 0) {
